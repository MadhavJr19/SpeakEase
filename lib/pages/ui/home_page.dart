...
class _MyHomePageState extends State<MyHomePage> with SingleTickerProviderStateMixin {
  ...
  List<Appointment> _appointments = []; // Add list to store appointments
  bool _isLoadingAppointments = true; // Add loading state for appointments
  ...
  void initState() {
    ...
    _loadAllLevelsProgress(); // Load progress for all levels
    initializeAppointments().then((_) => _loadAppointments()); // Load appointments
  }
  ...
  // Check progress of a level task
  Future<double> _getLevelProgress(String levelTask) async {
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    double progress = 0.0;

    // Extract the level number from the task (e.g., "Complete Level 1" -> 1)
    final levelNumber = int.tryParse(levelTask.replaceAll('Complete Level ', '')) ?? 0;
    if (levelNumber == 0) return 0.0; // Invalid level number

    // Map level number to the corresponding SharedPreferences key
    String key;
    switch (levelNumber) {
      case 1:
        key = 'wordRecognized';
    ...
    // Retrieve the progress for the level
  ...
  // Check if all tasks in the appointment are completed
  Future<bool> _areAllTasksCompleted(Appointment appointment) async {
    if (appointment.toDo.isEmpty) {
      return true; // If there are no tasks, consider it completed
    }
  ...
  Widget _buildPremiumAppointmentReminder() {
    ...
    // At this point, upcomingAppointment is guaranteed to be non-null
    ...
    return FutureBuilder<bool>(
      ...
      builder: (context, snapshot) {
        ...
        // Set gradient based on task completion
        final gradient = allTasksCompleted
            ? LinearGradient(
          begin: Alignment.topLeft,
          end: Alignment.bottomRight,
          colors: [
            Color(0xFF66BB6A).withOpacity(0.9), // Green for completed
          ...
          colors: [
            Color(0xFFFF7043).withOpacity(0.9), // Red-orange for incomplete
          ...
          child: Container(
            ...
            child: Stack(
              children: [
                Container(
                  decoration: BoxDecoration(
                    gradient: gradient, // Use the dynamic gradient
                    borderRadius: BorderRadius.circular(24),
                    boxShadow: [
                      BoxShadow(
  ...
  Widget _buildEnhancedLevelContainer(
      ...
      Function onTap, {
        ...
        required double progress, // Progress from 0.0 to 1.0
      }) {
    final Color accentColor = const Color(0xFFFF8C00); // Match LevelOneScreen
        ...
        child: Stack(
          ...
          children: [
            ClipRRect(
              ...
              child: Container(
                height: 260, // Adjusted to fit compact progress bar
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 10),
                        if (isUnlocked) // Show progress bar only if unlocked
                          SizedBox(
                            width: double.infinity,
                            child: Padding(
                                  ...
                                  Stack(
                                    children: [
                                      ...
                                      AnimatedContainer(
                                        ...
                                        height: 15,
                                        width: 100 * progress.clamp(0.0, 1.0), // Responsive to progress
                                        decoration: BoxDecoration(
